...
# BLEU scorer (only) when skipping full COCOEvalCap
try:
    from pycocoevalcap.bleu.bleu import Bleu as CocoBleu
except Exception:
    CocoBleu = None
# Track availability of COCO API
...
def getCOCO(dataset):
    """
    Pick the right ground-truth JSON for evaluation based on dataset name/path.
    """
    if not COCO_API_AVAILABLE:
        raise ImportError("coco-caption / pycocotools not available; cannot run language_eval")
    dataset_l = dataset.lower()
    if 'ktvic' in dataset_l or 'coco20k' in dataset_l or 'coco_20k' in dataset_l:
        annFile = 'data/coco_20k/val_data.json'
    elif 'test_images' in dataset_l:
        # Allow custom test jsons (already Karpathy style with captions)
        annFile = dataset
    elif 'coco' in dataset_l:
        annFile = 'coco-caption/annotations/captions_val2014.json'
    elif 'flickr30k' in dataset_l or 'f30k' in dataset_l:
        annFile = 'data/f30k_captions4eval.json'
    elif os.path.isfile(dataset):
        annFile = dataset
    else:
        raise ValueError('Unknown dataset for COCO evaluation: %s' % dataset)
    coco = COCO(annFile)
    # Some custom annotations may miss optional "info" field; ensure it exists for loadRes
...
def language_eval(dataset, preds, preds_n, eval_kwargs, split):
    ...
    reference_json = eval_kwargs.get('language_eval_json') if eval_kwargs is not None else ''
    coco = getCOCO(reference_json or dataset)
...
def eval_split(model, crit, loader, eval_kwargs={}):
    ...
    skip_predictions = eval_kwargs.get('skip_predictions', False)  # if True: only compute loss, no caption generation
    ...
    total_images = num_images if num_images != -1 else len(loader.dataset.split_ix[split])
    pbar = tqdm(total=total_images, desc=f'Evaluating {split}', leave=False)
    ...
    while True:
        ...
        if num_images != -1:
            ...
            num_images = ix1
        overflow = max(0, n - ix1)
        remove_count = min(overflow, len(predictions))
        for _ in range(remove_count):
            predictions.pop()

        pbar.update(min(len(data['infos']), pbar.total - pbar.n))
        if verbose:
    ...
    if lang_eval == 1 and not skip_predictions:
