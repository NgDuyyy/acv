...
except ImportError:  # lmdbdict is optional; only needed for .lmdb feature stores
    lmdbdict = None
    DUMPS_FUNC = LOADS_FUNC = None
import os
import numpy as np
...
class HybridLoader:
    ...
    def __init__(self, db_path, ext, in_memory=False):
        ...
        if db_path.endswith('.lmdb'):
            if lmdbdict is None:
                raise ImportError("lmdbdict is required to read LMDB feature files. Please install it via 'pip install lmdbdict'.")
            self.db_type = 'lmdb'
            ...
            self.feat_file = torch.load(db_path, weights_only=False)
...
class Dataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        train_name = getattr(opt, 'train_split', 'train')
        val_name = getattr(opt, 'val_split', 'val')
        test_name = getattr(opt, 'test_split', 'test')
        for ix in range(len(self.info['images'])):
            ...
            elif img['split'] == train_name:
                self.split_ix['train'].append(ix)
            elif img['split'] == val_name:
                self.split_ix['val'].append(ix)
            elif img['split'] == test_name:
                self.split_ix['test'].append(ix)
...
class DataLoader:
    def __init__(self, opt):
        ...
        self.num_workers = max(0, int(getattr(self.opt, 'num_workers', 4)))
        self.pin_memory = bool(getattr(self.opt, 'pin_memory', 1))
