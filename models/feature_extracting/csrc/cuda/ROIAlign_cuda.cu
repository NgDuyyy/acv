...
__global__ void RoIAlignForward(const int nthreads, const T* bottom_data,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    for (int iy = 0; iy < roi_bin_grid_h; iy++) {
      const T y = roi_start_h + ph * bin_size_h + (iy + 0.5f) * bin_size_h / roi_bin_grid_h;
      for (int ix = 0; ix < roi_bin_grid_w; ix++) {
        const T x = roi_start_w + pw * bin_size_w + (ix + 0.5f) * bin_size_w / roi_bin_grid_w;
...
__global__ void RoIAlignBackwardFeature(const int nthreads, const T* top_diff,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    T* offset_bottom_diff = bottom_diff + (roi_batch_ind * channels + c) * height * width;
    const T top_diff_this_bin = top_diff[(n * channels + c) * pooled_height * pooled_width + ph * pooled_width + pw];
...
at::Tensor ROIAlign_forward_cuda(const at::Tensor& input,
  ...
  dim3 grid(std::min((int)CeilDiv(output_size, 512), 4096));
  dim3 block(512);
...
at::Tensor ROIAlign_backward_cuda(const at::Tensor& grad,
  ...
  dim3 grid(std::min((int)CeilDiv((int)grad.numel(), 512), 4096));
  dim3 block(512);
