...
class Meteor:
    ...
    def __init__(self):
        ...
        self.meteor_cmd = ['java', '-jar', '-Xmx2G', METEOR_JAR, \
                '-', '-', '-stdio', '-l', 'en', '-norm']
        # Force UTF-8 so we can handle non-ASCII captions (Windows default cp1252 would break)
        self.meteor_p = subprocess.Popen(self.meteor_cmd, \
                cwd=os.path.dirname(os.path.abspath(__file__)), \
                stdin=subprocess.PIPE, \
                stdout=subprocess.PIPE, \
                stderr=subprocess.PIPE,
                env=self.env, universal_newlines=True, encoding='utf-8', errors='ignore', bufsize=1)
